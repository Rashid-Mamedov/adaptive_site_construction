...
const swiper = new Swiper('.swiper', {
  ...
  document.querySelectorAll('.section-4__btn').forEach(function(tabsBtn){
    ...
    document.querySelectorAll('.section-4__conteiner').forEach(function(tabsBtn){
tabsBtn.classList.remove('conteiner__active')});
document.querySelector(`[data-target="${path}"]`).classList.add('conteiner__active');
});
});
...
new Accordion('.accordion-list',{
  ...
  document.querySelector('.header__form__input__btn__close').addEventListener('click',function(){
    document.querySelector('.header__form').classList.remove('header__form__show')
  })
